...
const Scope2 = () => { 
  ...
  const tabs = {
    electricity: {
      id: "electricity", label: "Consommation d'électricité", headers: ["Consommation électrique (kWh)", "Action"], fields: [
        {
          name: "yearlyConsumption", label: "Consommation électrique (kWh)", type: "number", placeholder: "Consommation annuelle en kWh", required: true,
        },
        {
          name: "country", label: "Pays", type: "select", placeholder: "Sélectionner un pays", required: true, options: [
            "Sweden", "Lithuania", "France", "Austria", "Latvia",  "Finland", "Slovakia", "Denmark", "Belgium", "Croatia", "Luxembourg", "Slovenia", "Italy", "Hungary", "Spain", "United Kingdom", "Romania", "Portugal", "Ireland", "Germany", "Bulgaria", "Netherlands", "Czechia", "Greece", "Malta", "Cyprus", "Poland", "Estonia",
          ].map((country) => ({ value: country, label: country })),
        },
      ],
    },
    heating: {
      id: "heating", label: "Chauffage", headers: ["Nom", "Type", "Énergie Consommée (kWh)", "Action"], fields: [
        { 
          name: "name", label: "Nom", type: "text", placeholder: "Nom de l'équipement",required: true 
        },
        {
          name: "type", label: "Type", type: "select", placeholder: "Sélectionner un type", required: true, options: [ { value: "Electric Heating", label: "Electric Heating" }, { value: "District Heating", label: "District Heating" },
          ],
        },
        {
          name: "energy", label: "Énergie Consommée (kWh)", type: "number", placeholder: "Énergie consommée", required: true,
        },
      ],
    },
    cooling: {
      id: "cooling",label: "Refroidissement",headers: ["Nom", "Type", "Énergie Consommée (kWh)", "Action"],fields: [
        { 
          name: "name",  label: "Nom",  type: "text",  placeholder: "Nom de l'équipement", required: true 
        },
        {
          name: "type", label: "Type", type: "select", placeholder: "Sélectionner un type", required: true, options: [ { value: "Electric Cooling", label: "Electric Cooling" }, { value: "District Cooling", label: "District Cooling" },
          ],
        },
        {
          name: "energy", label: "Énergie Consommée (kWh)", type: "number", placeholder: "Énergie consommée", required: true,
    ...
    const fetchUser = async () => {
      try {
        ...
        if (!companyData?.data?._id) {
          setError("No company associated with this account. Please create a company first.");
          return;
        }
  ...
  const fetchScope2Data = async () => {
    try {
      const [electricityRes, heatingRes, coolingRes] = await Promise.all([
        fetch(`http://localhost:4000/energy-consumption/${company._id}`).then(res => res.json()),
        fetch(`http://localhost:4000/heating/${company._id}`).then(res => res.json()),
        fetch(`http://localhost:4000/cooling/${company._id}`).then(res => res.json()),
      ]);
      
      setScope2Data({
        electricity: electricityRes.data || { yearlyConsumption: 0, emissions: 0, country: null },
  ...
  const handleSubmit = async (e) => {
    ...
    if (!company || !company._id) {
      const errorMsg = "Company information not available. Please refresh and try again.";
    ...
    return (
      <div className="table-container p-5">
          ...
          <nav className="d-flex justify-content-center mt-4">
            <ul className="pagination">
              ...
              <li className={`page-item ${currentPage === totalPages ? "disabled" : ""}`}>
                <button
                  className="page-link" onClick={() => setCurrentPage(currentPage + 1)} disabled={currentPage === totalPages} >
                  Suivant
                </button>
              </li>
            </ul>
        ...
        <div className="d-flex justify-content-end mt-4">
          <button 
            type="button"  className="btn btn-primary"  onClick={() => toggleModal(true)}  >
    ...
    return currentTab.fields.map((field, index) => (
      <div className="mb-3" key={index}>
        ...
        {field.type === "select" ? (
          ...
          <input
            type={field.type} className="form-control" name={field.name} placeholder={field.placeholder} onChange={handleInputChange} value={formData[field.name] || ""} required={field.required}
          />
        )}
      </div>
    ));
        ...
        <div className="modal-dialog modal-lg" role="document">
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button
                type="button" className="btn-close" onClick={() => toggleModal(false)} aria-label="Close"
              ></button>
            </div>
            <form onSubmit={handleSubmit}>
              ...
              <div className="modal-footer">
                <button
                  type="button" className="btn btn-link link-secondary" onClick={() => toggleModal(false)}
                > Annuler
                </button>
      ...
      <div className="card mb-3">
        <div className="card-body">
          ...
          <p className="card-subtitle">
            <strong className="text-primary">Émissions indirectes</strong> provenant de
            sources détenues ou contrôlées par une organisation.
          </p>
        </div>
      </div>
      <div className="card">
