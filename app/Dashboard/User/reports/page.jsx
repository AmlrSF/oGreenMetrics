...
import {
Calendar,BarChart2,UserX,UserCheck,Trash2,FileTextIcon,
} from "lucide-react";
const Reporting = () => {
...
const [formData, setFormData] = useState({ name: "", description: "", scope1: false, scope2: false, scope3: false, Year: "", includeCharts: "yes", detailLevel: "summary",});
...
<span className="form-selectgroup-label">Scope 3</span>
            ...
            <div className="row mb-3">
              <div className="col-6">
                ...
                <select
                  name="Year" className="form-control" value={formData.Year} onChange={handleInputChange}>
            ...
            <div className="mb-3">
              ...
              <div className="form-selectgroup">
                <label className="form-selectgroup-item">
                  <input
                    type="radio" name="detailLevel" value="summary" className="form-selectgroup-input" checked={formData.detailLevel === "summary"} onChange={handleInputChange}/>
                  <span className="form-selectgroup-label">Summary</span>
                </label>
                <label className="form-selectgroup-item">
                  <input
                    type="radio" name="detailLevel" value="detailed" className="form-selectgroup-input" checked={formData.detailLevel === "detailed"} onChange={handleInputChange}/>
                  <span className="form-selectgroup-label">Detailed</span>
                </label>
              </div>  </div> </div>
          <div className="modal-footer">
            <button
              type="button"className="btn btn-link link-secondary"onClick={() => setModalOpen(false)}>Cancel </button>
            <button type="button" className="btn btn-primary ms-auto" onClick={handleSubmit}>Generate Report </button>
          </div> </div> </div>
      <div
        style={{ zIndex: 1040 }}  className="modal-backdrop fade show" onClick={() => setModalOpen(false)}
      ></div>
    </div>  )}
  <div className="py-10 mb-5 d-flex leading-[0.1] border-b  justify-content-center align-items-start">
    <div>
      <h3 className="text-[30px] font-bold" style={{ color: "#263589" }}>Reporting </h3>
      <div className="card-subtitle">Generate and manage your environmental impact reports
      </div>
    </div>
    <button  type="button" className="btn btn-primary ms-auto" onClick={() => setModalOpen(true)} > Generate Report </button>
  </div>
  <div className="card pt-5">
    <div className="card-body border-bottom py-3">
      <div className="d-flex">
        <div className="text-secondary d-flex align-items-center">Show
          <div className="mx-2 d-flex gap-2">
            <select
              className="form-select form-select-sm"
              value={scopeFilter}
              onChange={(e) => setScopeFilter(e.target.value)} >
              <option value="all">All Scopes</option> <option value="scope1">Scope 1</option> <option value="scope2">Scope 2</option> <option value="scope3">Scope 3</option>
            </select>
            <select
              className="form-select form-select-sm" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} >
      ...
      <div className="card-body">
        ...
        <div className="table-responsive">
          <table className="table table-vcenter card-table">
            <thead>
              <tr>
                <th>Report Name</th> <th>Description</th> <th>Period</th> <th>Scopes</th> <th>Charts</th> <th>Detail Level</th> <th>Status</th> <th>Action</th>
              </tr>
            </thead>
            <tbody>
              {reports?.length > 0 ? (
                    ...
                    <tr key={data._id}>
                      <td>
                        <div className="d-flex align-items-center">
                          <span className="avatar avatar-md bg-blue-lt text-blue me-2">  <FileTextIcon size={18} />  </span>
                          <div className="flex-fill">
                            <div className="font-weight-medium">  {data.name} </div> </div>  </div> </td>
                      <td className="text-secondary">
                        {data.description.length > 30 ? `${data.description.slice(0, 30)}...` : data.description}
                      </td>
                      <td>
                        <div className="d-flex align-items-center"> <Calendar size={16} className="me-1" /> <span>{data.Year}</span> </div>
                      </td>
                      <td> <span className="badge bg-purple-lt">  {getScopes(data)}  </span>  </td>
                      <td>
                        <span className="badge flex items-center bg-blue-lt">
                          <div className="flex"> <BarChart2 size={14} className="me-1" /> {data.includeCharts ? "Yes" : "No"}  </div>
                        </span>
                      </td>
                      <td>
                        <span className="badge bg-green-lt"> {data.detailLevel} </span>
                      </td>
                      <td>
                        <span
                          className={`badge ${
                            data.status === "pending"  ? "bg-yellow-lt" : data.status === "completed"  ? "bg-success-lt"  : "bg-secondary-lt"
                          }`}>
                      ...
                      <td>
                        <div className="btn-list flex-nowrap">
                          <button className={`btn btn-ghost-${ data.status === "pending"  ? "danger" : "success"  } btn-icon`}  >
                            {data.status === "pending" ? ( <UserX size={18} /> ) : (  <UserCheck size={18} />  )}
                          </button>
                          <button
                            onClick={() => deleteReport(data?._id)} className="btn btn-ghost-danger btn-icon">
                            <Trash2 size={18} />
                          </button>
                        </div> </td> </tr> )) ) : (
                <tr>
                  <td colSpan="8" className="text-center text-muted">  No reports available. </td> </tr>  )}
            </tbody>
          </table>
        </div>
        <div className="card-footer d-flex align-items-center">
          ...
          <ul className="pagination m-0 ms-auto">
            <li
              ...
              <button
                className="page-link" onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1)) } >
                <svg
            ...
            <li className="page-item active">
              <span
                className="page-link" style={{ backgroundColor: "#263589", borderColor: "#263589",}} >
                {currentPage}
              </span>
            </li>
            <li
              ...
              <button className="page-link" onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages)) } >
                <svg
                  xmlns="http://www.w3.org/2000/svg" className="icon" width="24" height="24" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round"> <path stroke="none" d="M0 0h24v24H0z" fill="none" />  <path d="M9 6l6 6l-6 6" />
